...
- 2026-01-27: fix agent-browser skill doc drift: remove reference to missing `./browser-start.js`, add install steps + lowest-friction `agent-browser open` flow; extend `pnpm vibe:doctor` to enforce this. Ran `pnpm check` clean. Status: done.
- 2026-01-27: security hardening (P1.1/P1.2): bind assist server to 127.0.0.1 by default; require VIBE_ASSIST_TOKEN if binding externally; client sends token if present in env; fixed demo fetch to be base-aware. Ran pnpm check clean. Status: done.
- 2026-01-27: documentation refresh (P0/P2): added root README.md; updated TEMPLATE_ARCHITECTURE.md to reflect pnpm dev default; updated AGENTS.md with pointers to invariants/checks. Updated TODO.md. Ran pnpm check clean. Status: done.
...
- 2026-01-28: enhance test coverage: added theme toggle accessibility test in App.test.tsx to verify agent ergonomics (aria-labels). Verified via pnpm check. Status: done.
